...
if (isset($_SESSION['id']) && isset($_SESSION['user_name'])) {
    ...
    if (isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_new_password'])) {
        ...
        if (empty($old_password)) {
            ...
            header("Location: change_password.php?error=The old password cannot be the new password");
            exit();
        } else if (strlen($new_password) < 8) {
            header("Location: change_password.php?error=Password should be at least 8 characters in length&$user_data");
            exit();
        } else if (!$uppercase || !$lowercase) {
            header("Location: change_password.php?error=Password should include at least one upper case letter.&$user_data");
            exit();
        } else {
